...
{
    internal class RandomTo4WeaponLegendaryItemEffect : LegendaryItemEffectWeaponBase
    {
        ...
        public override bool TryApplyWeaponEffect(ModItem modItem, OpenConstructionSet.Mods.Context.IModContext context)
        {
            if (!_gotEffects)
            {
                _effects = new LegendaryItemEffectApplyWeapons().EffectPatchers.Where(e => e.Name != Name).ToArray();
